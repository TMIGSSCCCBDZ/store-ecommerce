...
export const DeleteBillboardModal = () => {
    ...
    const modalOpened = isOpen && type === 'deleteBillboard'
    const {data} = useModal()
    const {store, billboard, storeId, billboardId} = data  as any
    ...
    const onSubmit = async() => {
        ...
        try {
            setIsLoading(true)
            await axios.patch(`/api/stores/${store ? store?.id : storeId}/billboards/${billboard ? billboard?.id : billboardId}/delete`)
            router.refresh()
            toast({
                variant:"default",
                title:'success',
                description:`${billboard?.label}  deleted successfully`
            })
    ...
    return (
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
            are you sure you want to delete <span className="font-bold text-destructive">{billboard?.label}</span>
            </DialogDescription>
          </DialogHeader>
